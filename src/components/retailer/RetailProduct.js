...
const RetailProduct=()=> {
  ...
  return (
    <div>
        ...
        <ul class="list-group">
        <li class="list-group-item">Availability-<b>BIOETHANOL: {availableProduct.bioethanol},BIODIESEL: {availableProduct.biodiesel}</b></li>
...
<Paper className={`${classes.paper}${d.senderType=="Depot"?"":"d-none"}`} component="form" onSubmit={async (event) => {
            ...
            {isPrivate && (<Box>
              ...
              <form className={`${receive=="Confirm Receipt"?"":"d-none"}${d.senderType=="Depot"?"":"d-none"}`}>
                <div>
            ...
            <div>
    <button variant="contained" type='submit' className={`${d.senderType=="Depot"?"":"d-none"} btn btn-warning`} onClick={productReceieved}>{receive}</button>
  </div>
            </FormControl>
